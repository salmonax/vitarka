...
**/
export default function mergeScratch(sheet, _parsley, skipMerge = false) {
  ...
  console.error('######## MERGING SCRATCH', skipMerge)
  ...
  const typeRegistry = {
    ...
    date: {
      ...
      after(originalLine, convertedDateString) {
        ...
        // NOTE 2021: This functionality is largely deprecated, as the
        // changes are never permanently moved into the sheet. For now, making
        // it depend on skipMerge
        if (!skipMerge) {
    ...
    task: {
      ...
      parse(converted, lastOfType) {
        return _parsley.parseTask(converted, -1, lastOfType.date, _parsley);
      },
    },
  };

  // This is intended for use with worker mode: on the worker-side, call with
  // skipMerge enabled, then call buildMergedPomsheet on the non-worker side.
  if (skipMerge) {
    const unmerged = convertScratch(sheet, typeRegistry);
    console.error('###### ASYNC MERGE FINISHED:', Date.now()-startMerge);
    return unmerged;
  }

  const {
    convertedScratchLines,
    // updatedScratch,
   } = convertScratch(sheet, typeRegistry);
  console.error('###### MERGE FINISHED:', Date.now()-startMerge);

  return {
    // TODO: Consider just returning the output.
    updatedPomsheet: buildMergedPomsheet(convertedScratchLines, _parsley),
    // updatedScratch, // Deprecated and previously unused
  };
}

export function buildMergedPomsheet(scratchData, _parsley) {
...
function convertScratch(sheet, registry) {
  ...
  return {
    // TODO: definitely consider changing this return signature
    convertedScratchLines: convertedLines,
    // VITARKA: largely deprecated.
